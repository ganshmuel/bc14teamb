...
function create_provider {
	...
	provider_id="$(curl -s -H 'Content-Type: application/json' --data "$payload" http://$host:8080/provider/ | jq -r .id)"
	echo $provider_id
}
...
echo "TEST: PUT /Provider, positive test"
...
payload="$(jq --null-input --arg nm "$provider_name" '{"name": $nm}')"
# Prepare URL
url="http://$host:8080/provider/$provider_id)"
# Send request
